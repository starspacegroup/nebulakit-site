...
vi.mock('$app/stores', () => {
	return {
		page: readable({
			url: new URL('http://localhost:4278/setup'),
			params: {},
			route: { id: '/setup' },
			status: 200,
			error: null,
...
describe('/setup - GitHub OAuth Setup', () => {
	...
	it('should display callback URL based on current origin', () => {
		render(SetupPage);
		// The mock page store uses 'http://localhost:4278' as origin
		...
		expect(callbackUrl.textContent).toContain('localhost:4278');
	});
	...
	it('should display homepage URL based on current origin', () => {
		...
		expect(homepageUrl?.textContent).toContain('http://localhost:4278');
	});
});
